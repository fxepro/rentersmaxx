...
body { font-family: 'Outfit', sans-serif; background: var(--cream); color: var(--text-dark); display: flex; overflow: hidden; font-size: 14px; }
...
.db-logo { padding: 20px 20px 16px; border-bottom: 1px solid var(--navy-border); flex-shrink: 0; }
.db-logo a { font-family: 'Fraunces', serif; font-size: 18px; font-weight: 700; color: var(--white); text-decoration: none; }
.db-logo a span { color: var(--terra-light); }
.db-nav { flex: 1; padding: 12px 10px; overflow-y: auto; }
.db-nav-section { margin-bottom: 20px; }
.db-nav-label { font-size: 10px; font-weight: 600; letter-spacing: 0.12em; text-transform: uppercase; color: rgba(255,255,255,0.25); padding: 0 10px; margin-bottom: 4px; display: block; }
.db-nav-item { display: flex; align-items: center; gap: 9px; padding: 8px 10px; border-radius: 8px; color: rgba(255,255,255,0.55); text-decoration: none; font-size: 13px; font-weight: 400; transition: all 0.15s; margin-bottom: 1px; }
...
.db-nav-item .ni { font-size: 14px; width: 18px; text-align: center; flex-shrink: 0; }
.db-nav-badge { margin-left: auto; font-size: 10px; font-weight: 600; background: var(--terra); color: var(--white); padding: 1px 6px; border-radius: 100px; }
...
.db-user { display: flex; align-items: center; gap: 9px; padding: 8px 10px; border-radius: 8px; margin-bottom: 2px; }
.db-avatar { width: 28px; height: 28px; border-radius: 50%; background: var(--terra); display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 700; color: var(--white); flex-shrink: 0; }
.db-user-name { font-size: 13px; font-weight: 500; color: var(--white); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.db-user-role { font-size: 11px; color: rgba(255,255,255,0.3); }
.db-logout { display: flex; align-items: center; gap: 9px; padding: 8px 10px; border-radius: 8px; color: rgba(255,255,255,0.35); font-size: 13px; transition: all 0.15s; cursor: pointer; background: none; border: none; width: 100%; font-family: 'Outfit', sans-serif; }
...
/* ── TOPBAR ── */
.db-topbar { height: var(--topbar); background: var(--white); border-bottom: 1px solid var(--cream-dark); display: flex; align-items: center; justify-content: space-between; padding: 0 24px; flex-shrink: 0; gap: 16px; }
.db-topbar-left { display: flex; align-items: center; gap: 12px; min-width: 0; }
.db-page-title { font-family: 'Fraunces', serif; font-size: 18px; font-weight: 500; color: var(--text-dark); white-space: nowrap; }
.db-breadcrumb { font-size: 13px; color: var(--text-light); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.db-topbar-right { display: flex; align-items: center; gap: 8px; flex-shrink: 0; }
.db-btn { display: inline-flex; align-items: center; gap: 6px; padding: 7px 14px; border-radius: 8px; font-family: 'Outfit', sans-serif; font-size: 13px; font-weight: 500; cursor: pointer; transition: all 0.15s; text-decoration: none; border: none; white-space: nowrap; }
...
/* ── CONTENT ── */
.db-content { flex: 1; overflow-y: auto; padding: 24px; background: var(--cream); }

/* ── CARDS ── */
.db-card { background: var(--white); border-radius: var(--radius); border: 1px solid var(--cream-dark); }
.db-card-header { display: flex; align-items: center; justify-content: space-between; padding: 16px 20px; border-bottom: 1px solid var(--cream-dark); gap: 12px; }
.db-card-title { font-size: 14px; font-weight: 600; color: var(--text-dark); }
.db-card-body { padding: 20px; }

/* ── STATS ── */
.db-stats { display: grid; grid-template-columns: repeat(4,1fr); gap: 14px; margin-bottom: 20px; }
.db-stat { background: var(--white); border-radius: var(--radius); padding: 18px 20px; border: 1px solid var(--cream-dark); }
.db-stat-label { font-size: 11px; color: var(--text-light); margin-bottom: 6px; text-transform: uppercase; letter-spacing: 0.06em; font-weight: 600; }
.db-stat-value { font-family: 'Fraunces', serif; font-size: 26px; color: var(--text-dark); letter-spacing: -0.02em; margin-bottom: 3px; line-height: 1; }
...
.db-table th { text-align: left; padding: 10px 16px; font-size: 11px; font-weight: 600; letter-spacing: 0.06em; text-transform: uppercase; color: var(--text-light); border-bottom: 1px solid var(--cream-dark); white-space: nowrap; background: var(--cream); }
.db-table td { padding: 13px 16px; border-bottom: 1px solid var(--cream-dark); color: var(--text-mid); vertical-align: middle; }
...
/* ── FLAG + NAME ── */
.db-flag-name { display: flex; align-items: center; gap: 8px; }
.db-flag { font-size: 16px; flex-shrink: 0; }
.db-name { font-weight: 500; color: var(--text-dark); }
.db-sub { font-size: 12px; color: var(--text-light); margin-top: 1px; }
...
.db-empty h3 { font-size: 16px; color: var(--text-dark); margin-bottom: 6px; }
.db-empty p { font-size: 13px; color: var(--text-light); margin-bottom: 20px; font-weight: 300; }
...
/* ── PROPERTY CARD ── */
.prop-card { background: var(--white); border: 1px solid var(--cream-dark); border-radius: var(--radius); padding: 20px; transition: all 0.15s; text-decoration: none; display: block; }
...
.prop-card-flag { font-size: 24px; }
.prop-card-name { font-size: 14px; font-weight: 600; color: var(--text-dark); margin-bottom: 2px; }
.prop-card-addr { font-size: 12px; color: var(--text-light); }
.prop-card-rent { font-family: 'Fraunces', serif; font-size: 20px; color: var(--text-dark); margin: 10px 0 4px; }
.prop-card-meta { font-size: 12px; color: var(--text-light); display: flex; gap: 12px; flex-wrap: wrap; }

/* ── FORM ── */
.db-form { display: flex; flex-direction: column; gap: 16px; max-width: 600px; }
.db-form-row { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
.db-form-group { display: flex; flex-direction: column; gap: 5px; }
.db-form-group label { font-size: 12px; font-weight: 600; color: var(--text-dark); }
.db-form-group label .req { color: var(--terra); }
.db-input, .db-select, .db-textarea { padding: 9px 12px; border-radius: 8px; border: 1px solid var(--cream-dark); background: var(--cream); font-family: 'Outfit', sans-serif; font-size: 13px; color: var(--text-dark); outline: none; transition: border-color 0.15s; width: 100%; }
...
.db-form-submit { padding: 10px 24px; border-radius: 9px; background: var(--terra); color: var(--white); font-family: 'Outfit', sans-serif; font-size: 14px; font-weight: 600; border: none; cursor: pointer; transition: all 0.15s; align-self: flex-start; }
.db-form-submit:hover { background: var(--terra-light); }
.db-form-hint { font-size: 12px; color: var(--text-light); font-weight: 300; }
.db-form-error { font-size: 12px; color: var(--red); margin-top: 2px; }

/* ── ALERT ── */
.db-alert { padding: 12px 16px; border-radius: 9px; font-size: 13px; margin-bottom: 16px; display: flex; align-items: center; gap: 10px; }
